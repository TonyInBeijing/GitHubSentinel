...
from hacker_news import HackerNews  # hacker news 任务
...
def hacker_news_job(hacker_news, report_generator):
    LOG.info("[开始执行定时任务] Hacker News")
    markdown_file_path = hacker_news.fetch_hackernews_top_stories()
    report, report_file_path = report_generator.generate_hacker_news_daily_report(
        markdown_file_path
    )
    LOG.info(f"[定时任务执行完毕]")
...
def main():
    ...
    github_client = GitHubClient(os.getenv("GITHUB_TOKEN"))  # 创建GitHub客户端实例
    ...
    hacker_news_job(hacker_news, report_generator)
