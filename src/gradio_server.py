...
from gs_resources_client import GSResourcesClient
...
gs_resources_client = GSResourcesClient() # 创建 GS Resources 客户端实例
...
def generate_gs_resources_report(model_type, model_name):
    config.llm_model_type = model_type
    ...
    markdown_file_path = gs_resources_client.export_top_stories()
    report, report_file_path = report_generator.generate_gs_resources_report(markdown_file_path)
...
with gr.Blocks(title="GitHubSentinel") as demo:
    ...
    # 创建 GS Resources 论坛热门帖子 Tab
    with gr.Tab("GS Resources 论坛热门帖子"):
        gr.Markdown("## GS Resources 论坛热门帖子")  # 添加小标题
        ...
        # 将按钮点击事件与导出函数绑定
        button.click(generate_gs_resources_report, inputs=[model_type, model_name], outputs=[markdown_output, file_output])
